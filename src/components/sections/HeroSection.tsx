    ...
    <section className="max-w-7xl mx-auto px-5 md:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-12 lg:gap-14 items-center min-h-[calc(100vh-80px)] py-16">
        ...
        <div className="order-2 lg:order-1">
          ...
          <motion.div
            {...fadeUp(0)}
            className="relative inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-7 bg-green-500/10 border border-green-500/20 backdrop-blur-md shadow-[0_0_15px_rgba(34,197,94,0.1)] overflow-hidden group"
          >
          ...
          <motion.p
            {...fadeUp(0.2)}
            className="text-brand-muted dark:text-dm-muted text-base md:text-[17px] leading-[1.75] max-w-[600px] mb-8"
          >
            {HERO_PROFILE.bio}
          </motion.p>
          ...
          >
            {HERO_STATS.map((stat, i) => (
              ...
              >
                <span className="font-syne text-[32px] font-extrabold text-brand-orange block">
                  {stat.value}
                </span>
                <p className="text-[12px] text-brand-muted dark:text-dm-muted mt-1 font-medium">
                  {stat.label}
                </p>
              </motion.div>
            ))}
        ...
        {/* ✅ Right column (Avatar) — shown FIRST on mobile, SECOND on desktop */}
        <motion.div
          className="flex flex-col gap-3.5 order-1 lg:order-2"
        ...
        >
          ...
          <div className="relative group w-full max-w-[380px] mx-auto">
            ...
            <div className="relative bg-brand-dark/80 backdrop-blur-2xl border border-white/10 rounded-[32px] p-10 text-center overflow-hidden hover:scale-[1.04] transition-all duration-500">
              ...
              <p className="text-[14px] text-brand-muted-2 mb-3">
                {HERO_PROFILE.role}
              </p>
              <p className="text-[12px] text-brand-muted-2 mb-6">
          ...
          <div className="flex items-center gap-2.5 bg-[rgba(255,109,31,0.06)] border border-[rgba(255,109,31,0.2)] rounded-xl px-4 py-3.5">
            ...
            <div>
              <p className="text-[13px] font-semibold text-brand-orange">
                {HERO_PROFILE.availabilityText}
              </p>
              <p className="text-[11px] text-brand-muted dark:text-dm-muted mt-0.5">
                {HERO_PROFILE.availabilitySub}
              </p>
            </div>
          </div>
          ...
          {/* Mini skills */}
          <div
            className={`border rounded-2xl p-4 ${isDark ? "bg-dm-card border-white/10" : "bg-white border-[rgba(34,34,34,0.1)]"}`}
          >
            <p className="text-[11px] text-brand-muted dark:text-dm-muted font-bold uppercase tracking-[0.6px] mb-3">
              Core Skills
            </p>
            {MINI_SKILLS.map((skill) => {
              ...
              return (
                ...
                >
                  <span className="text-[13px] text-brand-dark dark:text-dm-text font-semibold min-w-[90px]">
                    {skill.name}
                  </span>
