        ...
        <>
          {visibleProjects.map((project, i) => (
            ...
            >
              ...
              >
                <span className="text-[50px] relative z-10">{project.emoji}</span>
                {project.featured && (
              ...
              {/* Card body */}
              <div className="p-5">
                <Badge variant={project.catVariant as 'orange' | 'dark' | 'muted'} className="mb-2.5">
                  {project.catLabel}
                </Badge>

                <h3 className={`font-syne text-[18px] font-bold mb-2 ${isDark ? 'text-dm-text' : 'text-brand-dark'}`}>
                  {project.title}
                </h3>
                  ...
                  <div className="flex gap-2">
                    ...
                    {project.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`border-[1.5px] rounded-lg px-4 py-1.5 text-[12px] font-semibold font-jakarta transition-all hover:border-brand-orange hover:text-brand-orange ${isDark ? 'border-white/20 text-dm-muted' : 'border-[rgba(34,34,34,0.1)] text-brand-muted'}`}
                      >
                        GitHub
                      </a>
                    )}
